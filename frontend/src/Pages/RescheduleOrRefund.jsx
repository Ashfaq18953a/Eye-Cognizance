...
export default function RescheduleOrRefund() {
  ...
  const ApptInfo = () => (
    <div className="bg-gray-50 rounded-xl p-5 mb-8 border border-gray-200 text-sm space-y-2 shadow-inner">
      ...
      <div className="flex justify-between"><span className="text-gray-500">Type:</span> <strong className="text-gray-800 capitalize">{appt.consultation_type}</strong></div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#ECF3DF] to-[#d6eaf8] flex items-center justify-center p-6 font-sans">
      <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-lg w-full relative overflow-hidden">
        ...
        <div className="text-center mb-8">
            <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Emergency Leave</h1>
            <p className="text-gray-500 mt-2">Due to an emergency, this slot is no longer available.</p>
        </div>
        ...
        {actionDone === "already_rescheduled" || actionDone === "reschedule" ? (
            <div className="bg-green-50 rounded-2xl p-8 text-center border border-green-200">
                ...
                <p className="text-green-700 text-sm">
                    This appointment has already been rescheduled to a new slot. 
                    {actionDone === "reschedule" ? " Redirecting you now..." : " No further changes can be made through this link."}
                </p>
                <div className="mt-6 text-xs text-gray-400">If you need further help, please contact us.</div>
            </div>
            ...
            <div className="bg-blue-50 rounded-2xl p-8 text-center border border-blue-200">
                ...
                <p className="text-blue-700 text-sm">
                    A full refund of ₹{appt.amount} has been initiated to your original payment method. 
                    Expect it in 3-5 business days.
                </p>
                <div className="mt-6 text-xs text-gray-400">Confirmation email has been sent.</div>
            </div>
        ) : (
          <>
            <ApptInfo />
            ...
            {error && (
              <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-6 text-red-700 text-sm font-medium animate-shake">
                ⚠️ {error}
              </div>
            )}

            <div className="space-y-4">
                {/* Only show the action relevant to the link clicked */}
                {flow === "reschedule" ? (
                    <button
                        onClick={() => handleAction("reschedule")}
                        disabled={processing}
                        className="group relative w-full py-4 rounded-2xl bg-[#6A8E4F] hover:bg-[#5a7a41] text-white font-bold text-lg transition-all duration-300 shadow-lg hover:shadow-xl active:scale-95 disabled:opacity-50 disabled:pointer-events-none"
                    >
                        {processing ? "Processing..." : "📅 Reschedule for Free"}
                    </button>
                ) : (
                    <button
                        onClick={() => handleAction("cancel")}
                        disabled={processing}
                        className="group relative w-full py-4 rounded-2xl bg-red-600 hover:bg-red-700 text-white font-bold text-lg transition-all duration-300 shadow-lg hover:shadow-xl active:scale-95 disabled:opacity-50 disabled:pointer-events-none"
                    >
                        {processing ? "Processing..." : "💰 Confirm & Request Refund"}
                    </button>
                )}
                
                <p className="text-center text-xs text-gray-400 px-4">
                    Note: You can only take one action (either reschedule or refund). 
                    Once confirmed, this link will become inactive.
                </p>
            </div>
          </>
        )}
